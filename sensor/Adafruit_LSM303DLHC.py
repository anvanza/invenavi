...
class LSM303DLHC :
    ...
    def read_accelerations(self):
        ...
        accel_data = Obj3D()
        accel_data.x = (((xhi << 8) + xlo) >> 4)
        accel_data.y = (((yhi << 8) + ylo) >> 4)
        accel_data.z = (((zhi << 8) + zlo) >> 4)

        return accel_data

    def read_accelerations_g(self):
        """Returns accelerometer reading in G unit"""
        accel_data = self.read_accelerations()

        accel_val_3d = Obj3D()
        accel_val_3d.x = self.__twos_comp(accel_data.x, 12) * self.accel_factor
        accel_val_3d.y = self.__twos_comp(accel_data.y, 12) * self.accel_factor
        accel_val_3d.z = self.__twos_comp(accel_data.z, 12) * self.accel_factor
        return accel_val_3d

    def set_accelerometer_data_rate(self, rate):
        ...
        self.__set_ctrl_reg_1a()

    def set_accelerometer_low_power_mode(self, val):
    ...
    def __set_ctrl_reg_4a(self):
        param = (self.accelScale << 4) + (self.accel_hi_rez << 3)
        if (self.debug):
    ...
    def read_magnetics(self):
        ...
        mag_data = Obj3D()
        mag_data.x = (((xhi & 0x000F) << 8) + xlo)
        mag_data.y = (((yhi & 0x000F) << 8) + ylo)
        mag_data.z = (((zhi & 0x000F) << 8) + zlo)

        return mag_data

    def read_magnetics_gauss(self):
        """Returns magnetometer readings in gauss unit"""
        mage_data = self.read_magnetics()

        mag_val_3d = Obj3D()
        mag_val_3d.x = self.__twos_comp(mage_data.x, 12) * self.magFactor
        mag_val_3d.y = self.__twos_comp(mage_data.y, 12) * self.magFactor
        mag_val_3d.z = self.__twos_comp(mage_data.z, 12) * self.magFactor
        return mag_val_3d

    def read_magnetic_heading(self):
        """Return the heading in degrees"""
        mag_data = self.read_magnetics_gauss() # fixed thanks to aufder's input 20130210
        return math.degrees(math.atan2(mag_data.y, mag_data.x))

    def set_magnetometer_range(self, range=1.3):
    ...
    def set_temp_enabled(self, val=False):
        if val is False:
            self.tempEnabled = 0b0
        elif val is True:
        ...
        self.__set_cra_reg_m()

    def set_mag_data_rate(self, rate=15.0):
        if rate == 0.75:
    ...
    def read_temperature_celsius(self):
        """Returns temperature reading in celsius unit"""
